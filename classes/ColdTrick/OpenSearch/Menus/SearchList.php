...
class SearchList {
	...
	public static function registerSortMenu(\Elgg\Hook $hook) {
		...
		$title = elgg_echo('opensearch:menu:search_list:sort:title');
		...
		foreach ($items as $item) {
			$return[] = \ElggMenuItem::factory([
				'name' => $item,
				'text' => elgg_echo("opensearch:menu:search_list:sort:{$item}"),
		...
		switch ($type) {
			case 'group':
				$return[] = \ElggMenuItem::factory([
					'name' => 'members_count',
					'text' => elgg_echo("opensearch:menu:search_list:sort:member_count"),
					'href' => elgg_http_add_url_query_elements($url, [
						'sort' => 'member_count',
						'order' => 'desc',
						'offset' => null,
